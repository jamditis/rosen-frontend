...
| `archive-embeddings.bin` / `.json` | Quantized vectors and hash-bound ID index for non-social records |
| `archive-social-embeddings.bin` / `.json` | Quantized vectors and hash-bound ID index for social records; loaded only when meaning search is on |
| `relationship-review-report.json` | Curator review list for ruled endpoint mismatches, retypes, and inverse duplicates; regenerated by `npm run validate:graph` (issue #868) |
...
### Search-by-meaning vectors

After published records change, rebuild both vector pairs from the repository root:

```bash
npm run build-embeddings
npm run build-social-embeddings
```

The first command covers non-social records. The second command covers social
records. Both commands run the same model locally and write a binary plus its
hash-bound JSON index. The first run can download the model files.
...
Three things soften pure "always enforce" for existing data, all explicit
rather than silent:
...
- A type can carry `"endpointEnforcement": "deferred"` alongside an
  ...
  That means no curator has ruled on the endpoint types yet; the validator
  records the divergence instead of failing on it, while new rows are still
  expected to follow the declared types. Today only `Occurred At` is in this
  state.
- A type can instead carry `"endpointEnforcement": "reviewed"` (issue #868):
  a curator has ruled on the endpoint types (the 2026-08-27 comment on issue
  #737), so the rule itself is settled, but rows that predate the ruling and
  still don't fit it are not validation failures. `npm run validate:graph`
  collects every such row — across both accepted and held relationships, so
  `Founded By` and `Owned By` (which stay held; see below) are covered too —
  into the committed [`relationship-review-report.json`](relationship-review-report.json),
  grouped by type and sorted by relationship ID. Edit the registry or the
  source CSVs to change what lands there, not the report file itself; it
  regenerates every run.
- A type can carry `recommendedRetypeTo`, or an inverse pair can carry a
  `duplicateReviewPolicy`. Those settled, review-only actions land in the same
  report with an explicit reason and recommended action. The validator does
  not retype or remove the source row.
- `graph-validation-holds.json`'s `duplicateEdgeExceptions` names specific
  ...
  two entities) that are known to encode one fact twice. The registry records
  whether the curator has settled which label to keep; when it has, the rows
  to review appear in `relationship-review-report.json`. Any such duplicate
  not named here still fails validation; an
...
`"status": "deferred"` now covers two different states, both explicit in the
registry. `Founded By` and `Owned By` have settled direction but remain held
because they are outside the active extraction schema. `Created` has a settled
retype recommendation, also routed to the review report. `Covers` and the
not-yet-wired `Influenced` type (issues #344 / #548) still have unsettled
semantics. Every deferred row still requires an exact hold. The validator
enforces or reports only the rules the registry actually supplies; it does not
infer the missing ones. `directionalContradictions` inventories entity pairs
currently asserted in both directions under one type, while each type's ruling
note says whether those pairs are historical evidence or an open review task.
