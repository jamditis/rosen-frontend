...
- `npm run preview` serves the static bundle at `http://127.0.0.1:8000/` for a production-fidelity preview (same code paths as `pressthink.org/j/rosen-archive/`, with relative URLs auto-selected by `App.js` on localhost). Binds to loopback only by default. Override the port with `PREVIEW_PORT=8765 npm run preview`; override the bind address with `PREVIEW_HOST=0.0.0.0 npm run preview` if LAN access is intentional.
