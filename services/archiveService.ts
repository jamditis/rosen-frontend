...
import { ArchiveRecord, RawCsvRecord, Facets } from '../types';
...
const getEra = (dateStr: string): string => {
...
const DISSERTATION_RECORD: ArchiveRecord = {
...
export const fetchArchiveData = async (): Promise<{ records: ArchiveRecord[]; facets: Facets; autocompleteIndex: string[] }> => {
  ...
  const processRecord = (row: any, index: number, type: string): ArchiveRecord => {
      ...
      const author = (row.Author || row.author || 'Jay Rosen').trim();
  ...
  allRecords.push({ ...DISSERTATION_RECORD, relatedIds: [] } as ArchiveRecord);
