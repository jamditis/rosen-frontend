...
const RecordModal: React.FC<RecordModalProps> = ({ record, allRecords, isOpen, onClose, onNext, onPrev, onSelectRecord, hasPrev, hasNext, currentIndex, total }) => {
  const [isClosing, setIsClosing] = useState(false);
  const [showToast, setShowToast] = useState(false);
  const [toastMessage, setToastMessage] = useState('');
  const [relatedWorks, setRelatedWorks] = useState<ArchiveRecord[]>([]);
  ...
  useEffect(() => {
    if (isOpen && record && allRecords) {
      ...
      // Enhanced Related Works Logic
      // 1. Check explicit 'relatedIds' from extracted_relationships CSV
      // 2. Fallback to matching categories/concepts
...
const TagGroup: React.FC<{title: string, tags: string[]}> = ({title, tags}) => {
