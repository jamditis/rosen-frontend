...
try:
    ...
    logger.warning("Invalid GEMINI_API_RATE_LIMIT_CALLS value. Using default: 60")
    API_RATE_LIMIT_CALLS = 60

try:
    API_RATE_LIMIT_PERIOD = int(os.environ.get("GEMINI_API_RATE_LIMIT_PERIOD", "60"))
except ValueError:
    logger.warning("Invalid GEMINI_API_RATE_LIMIT_PERIOD value. Using default: 60")
    API_RATE_LIMIT_PERIOD = 60

# Validate configuration
if API_RATE_LIMIT_CALLS <= 0:
    logger.warning("GEMINI_API_RATE_LIMIT_CALLS must be positive. Using default: 60")
    API_RATE_LIMIT_CALLS = 60

if API_RATE_LIMIT_PERIOD <= 0:
    logger.warning("GEMINI_API_RATE_LIMIT_PERIOD must be positive. Using default: 60")
    API_RATE_LIMIT_PERIOD = 60
