...
def process_with_smart_corrector(limit=25, dry_run=False):
    ...
    """
    print("=" * 80)
    print(f"SMART DATA CORRECTOR - Processing Rows 27-42 (Multimedia Test)")
    print("=" * 80)
    ...
    # Get all records
    print(f"\n[2/4] Fetching rows 27-42...")
    all_records = worksheet.get_all_records()
    records = all_records[26:42]  # Rows 27-42 (indices 26-41)
    start_row = 27
    print(f"      Retrieved {len(records)} records (rows {start_row} to {start_row + len(records) - 1})")
    ...
    for i, record in enumerate(records, 1):
        row_num = start_row + i - 1  # Actual row number in sheet

        url = record.get('url', '')
        existing_raw_text = record.get('raw_text', '')

        if not url:
            print(f"\n[{i:2d}/{len(records)}] Row {row_num}: SKIP - No URL")
            continue

        print(f"\n[{i:2d}/{len(records)}] Row {row_num}: {url[:60]}...")
