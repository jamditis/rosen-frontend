...
- **B. Shelve it**
  ...
  - Risk, as originally written: it deletes the silent-failure fix along with the dead code. #628 has since put the fetch-and-parse fix in the live path's entity browser, so plain B no longer ships that masking behavior forward. It leaves two existing gaps unchanged: a successful response with a drifted payload shape still renders an empty browser, and the record modal still degrades without a signal. The modal behavior is deliberate and CI-pinned; the shape-drift behavior is not.
- **C. Fix in place, then shelve**
  - Do B. Before `fetchEntitiesData` accepts either a cached or freshly parsed payload, validate that `entities` is an array and that `records` is an array or `recordEntityMap` is an object. Route a validation failure through the same shaped `error` result as a fetch or parse failure. Also give `RecordModal` a way to say the entity index is down by reading that `error` field.
...
5. Keep the existing consumer-level rejected-load coverage (`tests/fail-loud-routes.test.js:64-70`) passing as the browser moves to the index, and add a loader-level test that a 200 with a malformed-but-parseable payload (a non-array `entities`) rejects instead of yielding an empty index. The index itself is already covered by `tests/entity-index.test.js`.
