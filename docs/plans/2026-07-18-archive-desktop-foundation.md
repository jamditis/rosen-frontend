...
- the standard route requests 77 resources totaling 27,580,720 decoded bytes after its existing background details preload and requests zero desktop assets;
- the equivalent five-sample garbage-collected standard-route median is 43,859,384 bytes of JavaScript heap, only 57,432 bytes (0.13%) above the pre-desktop baseline, with two additional DOM nodes and the same 285 event listeners. A cold desktop home uses 3,459,360 bytes because it does not load the corpus, while record-backed Start here uses 25,415,484 bytes after explicit garbage collection and still remains below the settled standard route because it does not trigger the details preload;
...
- `frontend/App.js` is 44,287 bytes, `ArchiveResults.js` is 8,714 bytes, `StartHerePage.js` is 23,199 bytes, the thin `DesktopStartPanel.js` adapter is 1,519 bytes, `frontend/services/viewState.js` is 7,626 bytes, and `frontend/services/router.js` is 3,568 bytes;
- a cold `#desktop` deep link requests 78 resources totaling 668,286 decoded bytes, including nine desktop assets totaling 95,173 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,068,475 decoded bytes without triggering the background details preload;
...
- the complete test suite passes 1,089 tests across 224 suites on rebased version `3.7.5`; the focused frontend suite passes 276 tests across 54 suites, and the production deploy-manifest suite passes all 40 focused tests;
...
The repository-wide preview audit covers 28 route states at mobile, tablet, and desktop sizes (84 rendered states) and reports the unchanged 33 rule-level baseline findings. The command retains that complete run by default and accepts `PREVIEW_AUDIT_VIEWPORT=mobile|tablet|desktop` for equivalent per-viewport shards on bounded runners. `#start`, the participation page, the standard selected-entity deep link, the open canonical dissertation detail panel, all 42 desktop-shell rows—including the useful unknown-app fallback, Start here, Selected findings, the selected-entity deep link, Read me, Tools, the explicit concurrent-window layout, the canonical record overlay, and the open report dialog—and the Winer method demonstration each contribute zero findings. The remaining findings are confined to previously maintained standard archive, standalone dissertation, reader, FAQ, and status-report surfaces.
