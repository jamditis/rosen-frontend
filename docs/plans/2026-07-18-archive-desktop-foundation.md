...
- dissertation nodes are keyboard reachable, its shortcuts act only while the map owns focus, and its non-modal detail panel moves focus to Close only after retained content renders, returns focus to the exact SVG node, then marks its off-screen closing state `aria-hidden` and inert so no hidden control can retake focus;
