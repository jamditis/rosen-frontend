...
- the standard route requests 77 resources totaling 27,571,771 decoded bytes after its existing background details preload and requests zero desktop assets;
- the equivalent five-sample garbage-collected standard-route median is 43,847,824 bytes of JavaScript heap, only 45,872 bytes (0.10%) above the pre-desktop baseline, with two additional DOM nodes and the same 285 event listeners. A cold desktop home uses 3,425,776 bytes because it does not load the corpus, while record-backed Start here uses 25,418,292 bytes after explicit garbage collection and still remains below the settled standard route because it does not trigger the details preload;
...
- a cold `#desktop` deep link requests 78 resources totaling 659,084 decoded bytes, including nine desktop assets totaling 94,920 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,059,273 decoded bytes without triggering the background details preload;
...
- People & ideas preserves research context in both directions at 375 and 1440 pixels: selecting Jay Rosen and a connected record emits the canonical `?record=RECORD-00903#desktop/entities` URL, Escape removes only the record query, keeps Jay Rosen's entity panel open, and returns focus to the exact invoking record button with zero overflow or accessibility findings;
...
- the complete test suite passes 1,085 tests across 224 suites on rebased version `3.7.5`; the focused frontend suite passes 272 tests across 54 suites, and the production deploy-manifest suite passes all 40 focused tests;
...
- a separate first-visit People & ideas check loads the canonical core corpus and entity graph, then reloads `#desktop/entities` offline under the root worker with 100 indexed entities visible; selecting Jay Rosen and opening a connected canonical record still works with zero page errors, horizontal overflow, or accessibility findings, and the same nine optional shell assets are warmed;
