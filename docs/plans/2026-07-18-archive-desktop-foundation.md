...
- a cold `#desktop` deep link requests 78 resources totaling 670,109 decoded bytes, including nine desktop assets totaling 95,820 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,070,298 decoded bytes without triggering the background details preload;
