...
- the standard route requests 77 resources totaling 27,570,315 decoded bytes after its existing background details preload and requests zero desktop assets;
...
- a cold `#desktop` deep link requests 78 resources totaling 659,216 decoded bytes, including nine desktop assets totaling 96,466 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,059,405 decoded bytes without triggering the background details preload;
...
- the complete test suite passes 1,081 tests across 224 suites on rebased version `3.7.5`; the focused frontend suite passes 270 tests across 54 suites, and the production deploy-manifest suite passes all 40 focused tests;
