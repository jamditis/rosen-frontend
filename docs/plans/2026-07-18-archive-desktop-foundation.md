...
- a cold `#desktop` deep link requests 78 resources totaling 648,674 decoded bytes, including nine desktop assets totaling 90,651 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,048,863 decoded bytes without triggering the background details preload;
