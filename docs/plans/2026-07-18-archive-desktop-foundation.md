...
- the standard route requests 60 same-origin resources totaling 27,496,268 uncompressed local bytes after its existing background details preload and requests zero desktop assets;
- the equivalent five-sample garbage-collected standard-route median is 43,891,168 bytes of JavaScript heap, 89,216 bytes (0.20%) above the pre-desktop baseline, with four additional DOM nodes (1,518 total) and the same 285 event listeners. A cold desktop home uses 3,471,440 bytes, 296 nodes, and 151 listeners because it does not load the corpus, while record-backed Start here uses 25,453,988 bytes, 674 nodes, and 174 listeners and still remains below the settled standard route because it does not trigger the details preload; all 15 samples have zero page errors;
...
- a cold `#desktop` deep link requests 66 same-origin resources totaling 675,907 uncompressed local bytes, including nine desktop assets totaling 99,971 source bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each request 68 same-origin resources totaling 14,076,096 uncompressed local bytes, make one `archive-core.json` request, and do not trigger `archive-details.json`; cold `#desktop/entities` requests 69 same-origin resources totaling 15,947,223 uncompressed local bytes and makes exactly one core plus one entity-index request;
