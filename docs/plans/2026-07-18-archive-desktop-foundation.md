...
- the standard route requests 77 resources totaling 27,566,281 decoded bytes after its existing background details preload and requests zero desktop assets;
...
- a cold `#desktop` deep link requests 78 resources totaling 652,749 decoded bytes, including nine desktop assets totaling 94,033 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,052,938 decoded bytes without triggering the background details preload;
