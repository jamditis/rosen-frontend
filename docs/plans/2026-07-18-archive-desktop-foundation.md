...
- the standard route requests 77 resources totaling 27,571,196 decoded bytes after its existing background details preload and requests zero desktop assets;
- the equivalent five-sample garbage-collected standard-route median is 43,846,264 bytes of JavaScript heap, only 44,312 bytes (0.10%) above the pre-desktop baseline, with two additional DOM nodes and the same 285 event listeners. A cold desktop home uses 3,425,160 bytes because it does not load the corpus, while record-backed Start here uses 43,638,776 bytes and still remains below the settled standard route because it does not trigger the details preload;
...
- a cold `#desktop` deep link requests 78 resources totaling 658,509 decoded bytes, including nine desktop assets totaling 94,920 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,058,698 decoded bytes without triggering the background details preload;
...
- the complete test suite passes 1,084 tests across 224 suites on rebased version `3.7.5`; the focused frontend suite passes 272 tests across 54 suites, and the production deploy-manifest suite passes all 40 focused tests;
