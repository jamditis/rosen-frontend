...
Post-change checks must show no `DesktopShell.js`, desktop registry, or desktop CSS request on the standard route. The desktop branch initially inherited version `3.7.5` when it rebased over PR #624, then moved to coordinated release version `3.7.6` during PR review so every `?v=` marker and the static service-worker cache key invalidate together.
...
Re-measured after the PR review fixes on 2026-07-19 from fresh 1440×900 Chromium contexts with service workers blocked. Network samples wait 2.5 seconds after initial network idle and settle again, count same-origin responses, and total their uncompressed local response bodies; heap samples use five fresh contexts per route and explicit garbage collection:

- the standard route requests 60 same-origin resources totaling 27,496,628 uncompressed local bytes after its existing background details preload and requests zero desktop assets;
- the equivalent five-sample garbage-collected standard-route median is 43,881,512 bytes of JavaScript heap, 79,560 bytes (0.18%) above the pre-desktop baseline, with four additional DOM nodes (1,518 total) and the same 285 event listeners. A cold desktop home uses 3,508,364 bytes, 296 nodes, and 151 listeners because it does not load the corpus, while record-backed Start here uses 25,454,084 bytes, 674 nodes, and 174 listeners and still remains below the settled standard route because it does not trigger the details preload; all 15 samples have zero page errors;
...
- `frontend/App.js` is 48,148 bytes, `ArchiveResults.js` is 8,714 bytes, `StartHerePage.js` is 23,222 bytes, the thin `DesktopStartPanel.js` adapter is 1,519 bytes, `frontend/services/viewState.js` is 7,626 bytes, and `frontend/services/router.js` is 3,568 bytes;
- a cold `#desktop` deep link requests 66 same-origin resources totaling 676,560 uncompressed local bytes, including nine desktop assets totaling 100,264 source bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each request 68 same-origin resources totaling 14,076,749 uncompressed local bytes, make one `archive-core.json` request, and do not trigger `archive-details.json`; cold `#desktop/entities` requests 69 same-origin resources totaling 15,947,876 uncompressed local bytes and makes exactly one core plus one entity-index request;
...
- opening a dissertation node now switches the shared viewport-fixed detail drawer to a clipped, container-positioned desktop variant. At 375, 768, and 1,440 pixels its close control is revealed before focus, exposes an explicit three-pixel high-contrast outline, the drawer remains within the dissertation surface, the permanent Standard archive taskbar control stays physically topmost, Escape returns to the exact node, and the open-detail axe scan has zero findings;
- optional-shell failure is also a permanent three-viewport fault path: a fresh browser context blocks `DesktopShell.js`, the error boundary visibly focuses “The standard archive is still ready,” and its touch-safe recovery control returns to the focused standard archive main landmark with no uncaught page error, horizontal overflow, or accessibility finding. The ordinary desktop module stays lazy and the standard route still requests zero desktop assets;
- a separate 320×568 compact-entry probe covers the width below the primary 375-pixel matrix. Ordinary desktop home focuses the visible launcher heading rather than the Welcome window below five shortcut rows; a compact unknown-app URL instead reveals and focuses its specific home fallback. Both states keep focus above the fixed taskbar with a visible indicator, no horizontal overflow, no browser errors, and no accessibility findings. Live wide-to-compact reflow applies the same focus rule;
- the Start-menu reset is activated with the keyboard at all three primary widths after opening a real Tools window. Reset removes the saved layout and every task button, settles the URL at desktop home, closes the menu, and visibly returns focus to the durable Start button instead of letting the ordinary last-window-close effect overwrite it with a shortcut;
- keyboard checks cover shortcut arrow navigation and activation, Start-menu focus entry/Escape/reset, background-window focus raising, minimize/restore/close and focus return, taskbar restoration, filter-drawer entry/Escape return, record-modal entry/return, guided in-shell navigation, browser Back to the exact guided title, unknown-app fallback, and mobile reflow without horizontal overflow. The permanent concurrent-window row also activates Archive from the taskbar, minimizes it into Analytics, then requires Back to remount and visibly focus Archive before Forward reconstructs the exact Analytics URL and title at all three widths; this caught and repaired a same-window-count race that had left focus in the now-background Analytics window or on `body`. Its desktop post-scan then opens the narrower Tools window over the topmost wide background window and performs a real pointer click on Analytics' physically exposed 44×44 close control, requiring the Tools URL and focus to remain active and the Analytics task to disappear; keyboard focus entering a background window still raises it before its control acts;
...
- foreground focus suppression is tied to an actually rendered record overlay, not merely the presence of a query parameter. At all three target widths, a permanent crafted Analytics deep link supplies both `?record=RECORD-00802` and `?entity=P0005`; because that app can display neither context, the shell canonicalizes both parameters away before interaction, renders no hidden record dialog, focuses the active Analytics title, has no horizontal overflow, and still round-trips its exact clean desktop URL through the standard Analytics view;
- a separate three-viewport fault path holds the canonical core response while directly loading `?record=NOT-A-RECORD#desktop/archive`. The selected ID does not count as an open overlay until it exists in loaded canonical data, so Archive owns visible focus while validation is pending; no phantom dialog appears, and release of the response removes the invalid parameter without moving focus away from Archive;
- desktop home and unknown-app URLs now retain saved windows in the taskbar without letting one visually cover the active home fallback. The permanent gated-route audit opens a real Tools window first, navigates to the guessed making-of URL, and requires the focused home title to be physically topmost at 375, 768, and 1440 pixels; activating a saved task still sends home behind the ordinary app stack;
...
- the complete test suite passes 1,106 tests across 226 suites on release version `3.7.6`; the focused frontend suite passes 286 tests across 55 suites, and the production deploy-manifest suite passes all 40 focused tests;
...
- live keyboard checks at 375, 768, and 1,440 pixels confirm both public entry controls—Start here's “Explore the archive desktop” and the Tools modal's “Archive desktop”—focus the desktop home title, while browser Back visibly focuses the exact canonical source view. Both entry/Back sequences are permanent in all three audit shards: the responsive standard trigger is named Tools below 640 pixels and More in the wider Tools row, the dialog close control receives initial focus, and Back returns to the exact standard URL plus its visibly outlined main entry;
...
- a fresh browser confirms the root worker installs `jrda-cache-3.7.6` and `jrda-data-3.7.6`, a standard visit caches zero desktop assets, first desktop use warms exactly nine unique versioned assets, and an offline `#desktop/findings` reload renders all three canonical records with no console errors;
- the root-scoped worker distinguishes SPA document navigations from controlled standalone pages before substituting its cached root. In a fresh browser that has never opened FAQ, offline `/faq/` returns an honest 503 instead of rendering archive home at the wrong URL, while an uncached `?q=public#desktop` navigation still receives the cached SPA document and reconstructs the desktop with no page error; executable worker tests cover both responses plus production root, `index.html`, FAQ, dissertation-reader, and feature-path classification;
...
- browser checks confirm one active full-width window at 375×812, 812×375 landscape, and a 720×450 200%-zoom equivalent; all have internal content scrolling, no horizontal overflow, and zero automated accessibility findings. The permanent concurrent-window audit now also reflows a focused three-window 1440×900 state live into 720×450, requires only Analytics to remain visible and focused with the taskbar in view, keyboard-scrolls the short Start menu to its final exit, runs a separate zero-finding axe scan, and expands back to the exact visible window stack without horizontal overflow. The main audit still fails on more than one pixel of document overflow in every one of the 51 desktop rows across phone, tablet, and desktop widths;
- a separate Chromium mobile context with `isMobile`, `hasTouch`, and a two-times device scale confirms single-tap Archive launch, active-window minimize, taskbar restore, Start-menu People & ideas launch, and the permanent Standard archive exit. The exit remains 118×44 CSS pixels, the final standard route has zero overflow or duplicate IDs, and the sequence emits no page error;
...
- direct Read me checks at 375 and 1440 pixels confirm the blocked making-of integration remains registry-only: neither “How it was made” nor a generic future-connections placeholder enters the rendered page, while reset remains visible and both layouts have zero overflow or accessibility findings. The permanent unknown-route row now also guesses `#desktop/making-of` after its accessibility scan and requires a generic unavailable notice, no label or editorial-gate text, no archive-data request, and focused desktop home at all three widths;
...
- the checked-in independent-review handoff gives an outside reviewer a reproducible first-impression, canonical research, spatial-memory, responsive, accessibility, offline, and failure-recovery sequence plus explicit approval fields. It does not self-approve the work and keeps making-of, merge, deployment, and live verification as separate gates; the coordinated version/cache stamp is included as a mechanical stale-cache safeguard, not as publication approval;
