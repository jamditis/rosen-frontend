...
- the equivalent five-sample garbage-collected standard-route median is 43,891,072 bytes of JavaScript heap, 89,120 bytes (0.20%) above the pre-desktop baseline, with four additional DOM nodes (1,518 total) and the same 285 event listeners. A cold desktop home uses 3,470,040 bytes, 296 nodes, and 151 listeners because it does not load the corpus, while record-backed Start here uses 25,452,548 bytes, 674 nodes, and 174 listeners and still remains below the settled standard route because it does not trigger the details preload; all 15 samples have zero page errors;
...
- a cold `#desktop` deep link requests 66 same-origin resources totaling 674,541 uncompressed local bytes, including nine desktop assets totaling 98,605 source bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each request 68 same-origin resources totaling 14,074,730 uncompressed local bytes, make one `archive-core.json` request, and do not trigger `archive-details.json`; cold `#desktop/entities` requests 69 same-origin resources totaling 15,945,857 uncompressed local bytes and makes exactly one core plus one entity-index request;
