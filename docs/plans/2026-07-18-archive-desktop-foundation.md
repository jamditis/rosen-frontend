...
- a cold `#desktop` deep link requests 78 resources totaling 659,478 decoded bytes, including nine desktop assets totaling 96,466 decoded bytes but no archive corpus;
- cold `#desktop/start` and `#desktop/findings` links each make one `archive-core.json` request, no duplicate corpus request, and settle at 14,059,667 decoded bytes without triggering the background details preload;
...
- the complete test suite passes 1,082 tests across 224 suites on rebased version `3.7.5`; the focused frontend suite passes 271 tests across 54 suites, and the production deploy-manifest suite passes all 40 focused tests;
