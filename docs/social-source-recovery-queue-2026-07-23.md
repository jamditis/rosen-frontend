...
- Original rows in queue: 54.
- Rows recovered from exact ATProto text and timestamp matches: 16.
- Rows still in queue: 38.
- Platform: Bluesky.
- Current unresolved-row `verified` value: `FALSE`.
- Current unresolved-row URL field: blank.
- Recovery evidence packet: `%TEMP%/rosen-bsky-source-recovery-20260723`.

## Recovered row IDs

These rows now have native `bsky.app` source URLs, `verified=TRUE`, and notes
recording the ATProto URI, CID, handle, and response hash.
